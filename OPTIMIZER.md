...
Each new matchup is first checked for determinism. A deterministic matchup reuses that probe battle as its sole simulation. Random matchups get 15 total battles, including the probe when one was required. An all-win sample is a provisional winning response and allows early exit. If a complete response search has no all-win, all-loss, or all-draw samples, every random candidate in that search is extended to 50 total battles. Estimates are cached across the entire chain; refinement adds the samples needed to reach 50. Except for matchups identified as deterministic, all reported certainty is **sampled, never proven**. `no-sampled-counter` means every opposing order examined has zero sampled wins, with draws allowed. Finite sampling can miss counters.

Before simulating a positioning, the optimizer projects end-turn effects for that order. Numeric effects already present on the submitted lineup are removed from their original recipients and reassigned according to the candidate order, so effects such as Monkey's front-most-friend buff are compared on the pets they would actually affect. Projected lineups are cached and returned in `finalPosition.playerPets` and `finalPosition.opponentPets`.
...
After the response search terminates, `steps` is condensed to the shortest route through the discovered counterpositioning graph that preserves the terminal response. Equally ranked responses form additional graph edges. Cycle results retain the shortest discovered cycle and report `cycle` indices for the condensed trace. Simulation and progress counters still describe the full search work.
