...
Responses maximize the responding side's win rate, breaking ties by draw rate and then heuristic order. `collectAllBestResponses: true` disables early exits and collects all equally ranked responses. `searchComplete` indicates whether all orders were checked; collected ties otherwise cover only the searched prefix. The search starts from the heuristic opponent order rather than evaluating all 14,400 pairs upfront.
