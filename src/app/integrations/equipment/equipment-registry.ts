...
type NoArgEquipmentConstructor = new (runtime: EngineContext) => Equipment;
type LogOnlyEquipmentConstructor = new (runtime: EngineContext, logService: LogService) => Equipment;
type LogAbilityEquipmentConstructor = new (runtime: EngineContext,
  logService: LogService,
  abilityService: AbilityService,
) => Equipment;
...
export const SPECIAL_EQUIPMENT_BUILDERS: {
  ...
  Mushroom: (deps) => new Mushroom(deps.logService.runtime, deps.logService, deps.petService),
  Popcorn: (deps) =>
    new Popcorn(deps.logService.runtime, deps.logService, deps.petService, deps.gameService),
  Seaweed: (deps) =>
    new Seaweed(deps.logService.runtime, deps.logService, deps.abilityService, deps.petService),
  'Cocoa Bean': (deps) => new CocoaBean(deps.logService.runtime, deps.logService, deps.petService),
  'Faint Bread': (deps) => new FaintBread(deps.logService.runtime, deps.logService, deps.petService),
  Radish: (deps) => new Radish(deps.logService.runtime, deps.logService),
};

export const SPECIAL_AILMENT_BUILDERS: {
  [key: string]: (deps: EquipmentRegistryDeps) => Equipment;
} = {
  Cowardly: (deps) => new Cowardly(deps.logService.runtime, deps.logService),
  Tasty: (deps) => new Tasty(deps.logService.runtime, deps.logService),
};
