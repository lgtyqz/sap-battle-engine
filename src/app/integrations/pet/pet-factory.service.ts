...
export class PetFactoryService {
  ...
  createPet(
    ...
    for (const gameServicePetCtor of PETS_NEEDING_GAMESERVICE_TYPES) {
      ...
      if (originalPet instanceof GameServicePet) {
        return new GameServicePet(this.runtime,
          this.logService,
          this.abilityService,
          petService,
          this.gameService,
    ...
    if (PETS_NEEDING_PETSERVICE_TYPES.includes(PetClass)) {
      return new PetClass(this.runtime,
        this.logService,
        this.abilityService,
        petService,
        parent,
  ...
  createPetFromForm(
    ...
    if (plan.builderKind === 'gameservice' && plan.PetClass) {
      ...
      const petInstance = new PetClass(this.runtime,
        this.logService,
        this.abilityService,
        petService,
        this.gameService,
  ...
  private resolveEquipment(
    ...
    const equipmentService = this.equipmentService;
    if (!equipmentService) {
      return null;
    }
