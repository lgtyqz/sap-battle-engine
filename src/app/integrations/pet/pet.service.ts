...
export class PetService {
  ...
  getRandomPet(parent: Player) {
    ...
    const tierDecision = this.runtime.random.chooseRandomOption(
      () => ({
        key: 'pet.random-tier',
        label: 'Random pet tier',
        options: tierOptions,
      }),
      () => this.runtime.random.getRandomInt(0, tierOptions.length - 1), tierOptions.length
    );
    ...
    const petDecision = this.runtime.random.chooseRandomOption(
      () => ({
        key: 'pet.random-pool',
    ...
    const expDecision = this.runtime.random.chooseRandomOption(
      () => ({
        key: 'pet.random-exp',
        label: `Random experience for ${pet}`,
        options: expOptions,
      }),
      () => this.runtime.random.getRandomInt(0, expOptions.length - 1), expOptions.length
    );
