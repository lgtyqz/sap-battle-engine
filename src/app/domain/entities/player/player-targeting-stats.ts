...
function pickPet(
  ...
  const decision = pets[0].runtime.random.chooseRandomOption(
    () => ({
      key,
      label: `${describePetRef(callingPet)} -> ${label} (${describePool(pets)}, ${pets.length} options)`,
      options: pets.map((pet) => buildPetOption(pet)),
    }),
    () => pets[0].runtime.random.getRandomInt(0, pets.length - 1), (pets).length
  );
  return {
...
function selectSortedPetsWithTieRandom(
  ...
  const shuffledPets = availablePets[0].runtime.random.shuffleCopy(availablePets);
...
export const getStrongestPet = (
  ...
  let maxStrength = -1;
  const pets = player.runtime.random.shuffleCopy(player.petArray.filter((p) => p.alive));
