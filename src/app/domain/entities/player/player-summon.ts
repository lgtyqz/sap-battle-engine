...
export const summonPet = (
  ...
  if (summoner && hasSilly(summoner)) {
    const targetTeamChoice = player.runtime.random.chooseRandomOption(
      () => ({
      ...
      () => player.runtime.random.getRandomInt(0, 1), 2
    );
    const targetTeam =
      targetTeamChoice.index === 0 ? player : player.opponent;
    const positionChoice = player.runtime.random.chooseRandomOption(
      () => ({
      ...
      () => player.runtime.random.getRandomInt(0, 4), ([0, 1, 2, 3, 4]).length
    );
