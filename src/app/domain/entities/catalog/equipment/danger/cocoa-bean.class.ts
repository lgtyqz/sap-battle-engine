...
export class CocoaBean extends Equipment {
  ...
  callback = (pet: Pet) => {
    ...
    pet.addAbility(
      new CocoaBeanAbility(this.runtime, pet, equipment, this.logService, this.petService),
    );
  };
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    for (let i = 0; i < this.equipment.multiplier; i++) {
      const choice = this.runtime.random.chooseRandomOption(
        ...
        () => this.runtime.random.getRandomInt(0, summonPool.length - 1), (summonPool).length
      );
