...
export class BrahmaChicken extends Pet {
  ...
  health = 3;
  initAbilities(): void {
    this.addAbility(new BrahmaChickenAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class BrahmaChickenAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    if (context.trigger === 'PostRemovalFaint') {
      const eligible = this.runtime.random.shuffle(
  ...
  copy(newOwner: Pet): BrahmaChickenAbility {
    return new BrahmaChickenAbility(this.runtime, newOwner, this.logService);
  }
}
