...
export class FarmerCrow extends Pet {
  ...
  health = 3;
  initAbilities(): void {
    this.addAbility(new FarmerCrowAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class FarmerCrowAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    for (const target of targets) {
      for (let i = 0; i < 3; i++) {
        feedCorncob(target, effectMultiplier);
      }
    }
  ...
  copy(newOwner: Pet): FarmerCrowAbility {
    return new FarmerCrowAbility(this.runtime, newOwner, this.logService);
  }
}
