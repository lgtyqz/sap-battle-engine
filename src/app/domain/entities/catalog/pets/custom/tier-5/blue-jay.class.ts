...
export class BlueJay extends Pet {
  ...
  health = 3;
  initAbilities(): void {
    this.addAbility(new BlueJayAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class BlueJayAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    if (context.trigger === 'PostRemovalFaint') {
      ...
      this.runtime.random.shuffle(friends);
  ...
  copy(newOwner: Pet): BlueJayAbility {
    return new BlueJayAbility(this.runtime, newOwner, this.logService);
  }
}
