...
export class Estemmenosuchus extends Pet {
  ...
  initAbilities(): void {
    this.addAbility(
      new EstemmenosuchusAbility(this.runtime, this, this.logService, this.abilityService),
    );
    super.initAbilities();
  }
...
export class EstemmenosuchusAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    for (let i = 0; i < summonCount; i++) {
      const token = new EstemmenosuchusFriend(this.runtime,
        this.logService,
        this.abilityService,
        owner.parent,
        buff,
  ...
  copy(newOwner: Pet): EstemmenosuchusAbility {
    return new EstemmenosuchusAbility(this.runtime,
      newOwner,
      this.logService,
      this.abilityService,
    );
