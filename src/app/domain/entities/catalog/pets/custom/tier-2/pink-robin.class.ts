...
export class PinkRobin extends Pet {
  ...
  override initAbilities(): void {
    this.addAbility(new PinkRobinAbility(this.runtime, this));
    super.initAbilities();
  }
}
...
export class PinkRobinAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    if (friends.length > 0) {
      const choice = this.runtime.random.chooseRandomOption(
        ...
        () => this.runtime.random.getRandomInt(0, friends.length - 1), (friends).length
      );
      const target = friends[choice.index];
      const abilityQueueService = this.runtime.services.abilityQueueService;
      const logService = this.runtime.services.logService;
  ...
  override copy(newOwner: Pet): PinkRobinAbility {
    return new PinkRobinAbility(this.runtime, newOwner);
  }
}
