...
export class Locust extends Pet {
  ...
  override initAbilities(): void {
    this.addAbility(
      new LocustAbility(this.runtime, this, this.logService, this.abilityService),
    );
    super.initAbilities();
  }
...
export class LocustAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    for (let i = 0; i < this.level; i++) {
      ...
      for (let foodIndex = 0; foodIndex < foodCount; foodIndex++) {
        ...
        const statFoodChoice = this.runtime.random.chooseLegacyRandomOption(
          () => ({
          ...
          () => this.runtime.random.getRandomInt(0, statFoods.length - 1), (statFoods).length
        );
  ...
  override copy(newOwner: Pet): LocustAbility {
    return new LocustAbility(this.runtime, newOwner, this.logService, this.abilityService);
  }
}
