...
export class Bakunawa extends Pet {
  ...
  health = 3;
  initAbilities(): void {
    this.addAbility(new BakunawaAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class BakunawaAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    for (const target of targets) {
      const sleepy = new Sleepy(this.runtime);
  ...
  copy(newOwner: Pet): BakunawaAbility {
    return new BakunawaAbility(this.runtime, newOwner, this.logService);
  }
}
