...
export class CoconutCrab extends Pet {
  ...
  health = 8;
  initAbilities(): void {
    this.addAbility(new CoconutCrabAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class CoconutCrabAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    if (context.trigger === 'Eat2') {
      const coconut = new Coconut(this.runtime);
  ...
  copy(newOwner: Pet): CoconutCrabAbility {
    return new CoconutCrabAbility(this.runtime, newOwner, this.logService);
  }
}
