...
export class HarpyEagle extends Pet {
  ...
  initAbilities(): void {
    this.addAbility(
      new HarpyEagleAbility(this.runtime, this, this.logService, this.petService),
    );
    super.initAbilities();
  }
...
export class HarpyEagleAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    const choice = this.runtime.random.chooseRandomOption(
      ...
      () => this.runtime.random.getRandomInt(0, tierOnePool.length - 1), (tierOnePool).length
    );
  ...
  copy(newOwner: Pet): HarpyEagleAbility {
    return new HarpyEagleAbility(this.runtime, newOwner, this.logService, this.petService);
  }
}
