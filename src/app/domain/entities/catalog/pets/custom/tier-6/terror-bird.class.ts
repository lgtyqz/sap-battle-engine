...
export class TerrorBird extends Pet {
  ...
  health = 3;
  initAbilities(): void {
    this.addAbility(new TerrorBirdAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class TerrorBirdAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    for (const pet of strawberryPets) {
      pet.givePetEquipment(new Peanut(this.runtime));
    }
  ...
  copy(newOwner: Pet): TerrorBirdAbility {
    return new TerrorBirdAbility(this.runtime, newOwner, this.logService);
  }
}
