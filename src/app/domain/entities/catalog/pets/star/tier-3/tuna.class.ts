...
export class Tuna extends Pet {
  ...
  initAbilities(): void {
    this.addAbility(new TunaAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class TunaAbility extends Ability {
  ...
  constructor(runtime: EngineContext, owner: Pet, logService: LogService, timesHurtOverride?: number) {
    super(runtime, {
      name: 'TunaAbility',
      owner: owner,
      triggers: ['Faint'],
  ...
  copy(newOwner: Pet): TunaAbility {
    return new TunaAbility(this.runtime,
      newOwner,
      this.logService,
