...
export class StorkAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    const summonDecision = this.runtime.random.chooseRandomOption(
      () => ({
      ...
      () => this.runtime.random.getRandomInt(0, summonPetPool.length - 1), (summonPetPool).length
    );
  ...
  copy(newOwner: Pet): StorkAbility {
    return new StorkAbility(this.runtime,
      newOwner,
      this.logService,
      this.abilityService,
      this.petService,
