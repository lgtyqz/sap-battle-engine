...
export class Flea extends Pet {
  ...
  health = 3;
  initAbilities(): void {
    this.addAbility(new FleaAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class FleaAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    for (let target of targetsResp.pets) {
      const equipment = new Weak(this.runtime);
  ...
  copy(newOwner: Pet): FleaAbility {
    return new FleaAbility(this.runtime, newOwner, this.logService);
  }
}
