...
export class HerculesBeetle extends Pet {
  ...
  initAbilities(): void {
    this.addAbility(
      new HerculesBeetleAbility(this.runtime, this, this.logService, this.abilityService),
    );
    super.initAbilities();
  }
...
export class HerculesBeetleAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    if (remainder > 0) {
      const shuffled = this.runtime.random.shuffle([...friends]);
  ...
  copy(newOwner: Pet): HerculesBeetleAbility {
    return new HerculesBeetleAbility(this.runtime,
      newOwner,
      this.logService,
      this.abilityService,
    );
