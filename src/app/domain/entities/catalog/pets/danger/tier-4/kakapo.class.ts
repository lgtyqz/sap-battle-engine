...
export class Kakapo extends Pet {
  ...
  initAbilities(): void {
    this.addAbility(new KakapoAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class KakapoAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    for (let i = 0; i < this.level; i++) {
      ...
      target.givePetEquipment(new Cowardly(this.runtime, this.logService));
  ...
  copy(newOwner: Pet): KakapoAbility {
    return new KakapoAbility(this.runtime, newOwner, this.logService);
  }
}
