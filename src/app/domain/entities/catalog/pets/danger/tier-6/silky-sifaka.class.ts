...
export class SilkySifaka extends Pet {
  ...
  initAbilities(): void {
    this.addAbility(
      new SilkySifakaAbility(this.runtime, this, this.logService, this.petService),
    );
    super.initAbilities();
  }
...
export class SilkySifakaAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    for (let targetPet of petsBehind) {
      ...
      const choice = this.runtime.random.chooseRandomOption(
        ...
        () => this.runtime.random.getRandomInt(0, sifakaPool.length - 1), (sifakaPool).length
      );
  ...
  copy(newOwner: Pet): SilkySifakaAbility {
    return new SilkySifakaAbility(this.runtime, newOwner, this.logService, this.petService);
  }
