...
export class Doberman extends Pet {
  ...
  health = 4;
  initAbilities(): void {
    this.addAbility(new DobermanAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class DobermanAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    coconutTarget.givePetEquipment(new Coconut(this.runtime));
    if (this.logService.isEnabled()) this.logService.createLog({
  ...
  copy(newOwner: Pet): DobermanAbility {
    return new DobermanAbility(this.runtime, newOwner, this.logService);
  }
}
