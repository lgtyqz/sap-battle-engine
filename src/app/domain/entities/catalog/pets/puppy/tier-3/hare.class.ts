...
export class Hare extends Pet {
  ...
  attack = 4;
  initAbilities(): void {
    this.addAbility(new HareAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class HareAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    const choice = this.runtime.random.chooseRandomOption(
      ...
      () => this.runtime.random.getRandomInt(0, equipmentPets.length - 1), (equipmentPets).length
    );
    ...
    const copiedEquipment = this.runtime.services.equipmentService
  ...
  copy(newOwner: Pet): HareAbility {
    return new HareAbility(this.runtime, newOwner, this.logService);
  }
}
