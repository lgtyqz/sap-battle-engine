...
export class Toucan extends Pet {
  ...
  health = 3;
  initAbilities(): void {
    this.addAbility(new ToucanAbility(this.runtime, this, this.logService));
    super.initAbilities();
  }
...
export class ToucanAbility extends Ability {
  ...
  private executeAbility(context: AbilityContext): void {
    ...
    for (let target of targets) {
      const newEquipmentInstance = this.runtime.services.equipmentService
  ...
  copy(newOwner: Pet): ToucanAbility {
    return new ToucanAbility(this.runtime, newOwner, this.logService);
  }
}
