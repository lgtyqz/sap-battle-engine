...
export function calculateDamage(
  ...
  if (snipe) {
    ...
    if (self.equipment instanceof HoneydewMelon) {
      attackEquipment = new HoneydewMelonAttack(self.runtime);
    } else if (self.equipment instanceof MapleSyrup) {
      attackEquipment = new MapleSyrupAttack(self.runtime);
    } else if (self.equipment instanceof Guava) {
      attackEquipment = new GuavaAttack(self.runtime);
    } else {
      attackEquipment =
  ...
  if (attackEquipment instanceof FortuneCookie && !snipe) {
    const fortuneDecision = self.runtime.random.chooseRandomOption(
      () => ({
      ...
      () => self.runtime.random.getRandomInt(0, 1), 2
    );
