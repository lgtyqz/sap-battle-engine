...
export function attackPet(
  ...
  let message: string;
  if (self.runtime.services.logService.isEnabled()) {
    if (jumpAttack) {
  ...
  if (attackEquipment instanceof Peanut && damage > 0) {
    if (self.runtime.services.logService.isEnabled()) self.createLog({
...
export function snipePet(
  ...
  } else if (defenseEquipment) {
    // Upstream's message helper also consumes defense equipment. Keep that mechanic
    // outside the optional event formatting path.
