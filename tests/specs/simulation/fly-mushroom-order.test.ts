...
describe('Fly + Mushroom Ordering', () => {
  it('spawns Zombie Fly in front of a Mushroom-respawned Snake', () => {
    ...
    const boardAfterZombieSpawn = logs.slice(zombieSpawn).find(log => log.board.opponent[0]?.name === 'Zombie Fly')!.board;
    expect(boardAfterZombieSpawn.opponent[0]?.name).toBe('Zombie Fly');
    expect(boardAfterZombieSpawn.opponent[1]?.name).toBe('Snake');
  });
  ...
  it('spawns level 2 Zombie Fly as 8/8', () => {
    ...
    const spawned = logs.slice(zombieSpawn).flatMap(log => log.board.opponent).find(pet => pet?.name === 'Zombie Fly');
    expect(spawned).toMatchObject({ attack: 8, health: 8, exp: 2 });
  });
